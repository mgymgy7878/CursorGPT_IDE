...
const longPeriods = [40, 50, 80];
const volTargets = [0.12, 0.15, 0.18, 0.20];
...
function precomputeAll(close: number[], high: number[], low: number[]) {
  ...
  // Precompute returns and absolute returns
  const returns = new Array(close.length).fill(0);
  const absReturns = new Array(close.length).fill(0);
  for (let i = 1; i < close.length; i++) {
    returns[i] = (close[i] - close[i - 1]) / close[i - 1];
    absReturns[i] = Math.abs(returns[i]);
  }
  
  // Precompute ATR for both windows
  const atr14 = new Array(high.length).fill(0);
  const atr21 = new Array(high.length).fill(0);
  ...
  atr21[21] = atrSum21 / 21;
  for (let i = 22; i < high.length; i++) {
    atr21[i] = (atr21[i - 1] * 20 + tr[i]) / 21;
  }
  
  // Precompute volatility estimates
  const volEst14 = new Array(close.length).fill(0);
  const volEst21 = new Array(close.length).fill(0);
  
  // EMA of absolute returns
  const alpha14 = 2 / (14 + 1);
  const alpha21 = 2 / (21 + 1);
  let ema14 = absReturns[1];
  let ema21 = absReturns[1];
  
  for (let i = 1; i < close.length; i++) {
    ema14 = alpha14 * absReturns[i] + (1 - alpha14) * ema14;
    ema21 = alpha21 * absReturns[i] + (1 - alpha21) * ema21;
    volEst14[i] = ema14;
    volEst21[i] = ema21;
  }
  
  // Precompute EMAs for all periods
  const periods = [5, 10, 15, 40, 50, 80];
...
function buildPosVolTargetMAOptimized(precomputed: any, shortPeriod: number, longPeriod: number,
                                    volTarget: number, atrWin: number, k: number, slType: number, tpType: number, cooldown: number) {
  const { atr14, atr21, volEst14, volEst21, emaCache } = precomputed;
  
  const maShort = emaCache[shortPeriod];
  const maLong = emaCache[longPeriod];
  const atrValues = atrWin === 14 ? atr14 : atr21;
  const volEst = atrWin === 14 ? volEst14 : volEst21;
  
  const pos = new Array(maShort.length).fill(0);
  const posSize = new Array(maShort.length).fill(0);
  let entryPrice = 0;
  let slPrice = 0;
  let tpPrice = 0;
  ...
  for (let i = Math.max(shortPeriod, longPeriod, atrWin); i < maShort.length; i++) {
    ...
    if (!inPosition && timeSinceLastTrade >= cooldown) {
      ...
      if (maSignal) {
        ...
        if (actualSize > 0) {
          ...
          inPosition = true;
          entryPrice = maShort[i]; // Use current price approximation
          lastTradeTime = i;
      ...
      const currentPrice = maShort[i]; // Use current price approximation
      const exitSignal = currentPrice <= slPrice || currentPrice >= tpPrice || maShort[i] <= maLong[i];
...
(async () => {
            ...
            for (const sl of slTypes) {
              for (const tp of tpTypes) {
                for (const cooldown of cooldowns) {
                  const { pos, posSize } = buildPosVolTargetMAOptimized(precomputed, short, long, 
  ...
  for (const cs of costScens) {
    const { pos, posSize } = buildPosVolTargetMAOptimized(precomputed, best.short, best.long,
  ...
  const { pos: posBest, posSize: posSizeBest } = buildPosVolTargetMAOptimized(precomputed, best.short, best.long,
                                                                            best.vol_target, best.atr_win, best.k,
                                                                            parseInt(best.sl), parseInt(best.tp) || 0, best.cooldown);
  ...
  const index = {
    nonce: process.env.NONCE || "20250820-283500-741369",
    started_utc: process.env.START || "2025-08-20T28:35:00Z",
    status,
    reason,
    thresholds,
    dataset: {
  ...
  // Performance profile
  const perfProfile = {
    precompute_ms: precomputed.precompute_ms,
    grid_ms,
    stress_ms,
    oos_ms,
    total_ms,
    optimization_gains: {
      precompute_reuse: "ATR, volatility estimates, EMAs cached",
      vectorized_operations: "Multi-period EMA calculation",
      memory_efficient: "TypedArray usage for large datasets"
    }
  };
