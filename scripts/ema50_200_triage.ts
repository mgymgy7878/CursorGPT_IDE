...
const shorts = [10, 20, 30, 40, 50];
const longs = [100, 150, 200, 250];
const slTpValues = [0, 0.005, 0.01, 0.02];
const costScens = [
...
(async () => {
    ...
    for (const long of longs) {
      ...
      for (const sl of slTpValues) {
        for (const tp of slTpValues) {
          const pos = buildPosEMA(close, short, long, sl, tp);
  ...
  const posBest = buildPosEMA(close, best.short, best.long, best.sl, best.tp);
  ...
  const index = {
    nonce: process.env.NONCE || "20250820-281500-258369",
    started_utc: process.env.START || "2025-08-20T28:15:00Z",
    status,
    reason,
    thresholds,
    dataset: {
  ...
  fs.writeFileSync(path.join(process.argv[2], "summary.csv"), [
    "short,long,sl,tp,sharpe,cagr_pct,max_dd_pct,trades,exposure_pct",
    ...base.sort((a, b) => b.sharpe - a.sharpe).map(x => `${x.short},${x.long},${x.sl},${x.tp},${x.sharpe},${x.cagr_pct},${x.max_dd_pct},${x.trades},${x.exposure_pct}`)
  ].join("\n"));
