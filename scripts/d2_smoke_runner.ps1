...
function Read-Metrics($p) {
  ...
  try {
    ...
    if ($obj -and $obj.counters -and $obj.gauges) { return @{ type="json"; data=$obj } }
    ...
    if ($msgs -or $stale) {
      return @{ type="prom"; data=@{ counters=@{ spark_ws_btcturk_msgs_total=$msgs }; gauges=@{ spark_ws_staleness_seconds=$stale } } }
    }
  } catch { return $null }
  return $null
}
...
if(-not $m1){ "port: N/A"; "msgs_total Δ: 0"; "staleness s: N/A"; "SMOKE: ATTENTION (endpoint down @3003/3004)"; exit 1 }

Start-Sleep -Seconds 4
$m2 = Read-Metrics $port
if(-not $m2){ "port: $port"; "msgs_total Δ: 0"; "staleness s: N/A"; "SMOKE: ATTENTION (2nd read failed @$port)"; exit 1 }
