/**
 * Market Data Golden Master Test
 *
...
test.describe('Market Data Golden Master', () => {
  ...
  test('market-data - default state', async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot('market-data-default.png', {
      fullPage: true,
      maxDiffPixels: 100, // Küçük farklara tolerans
    });
  });

  test('market-data - loading state', async ({ page }) => {
    await page.goto(`${BASE_URL}/market-data?state=loading`);
    await page.waitForLoadState('networkidle');

    await page.waitForSelector('[data-testid="status-bar"], .status-bar, header', { timeout: 5000 }).catch(() => {});

    await expect(page).toHaveScreenshot('market-data-loading.png', {
      fullPage: true,
      maxDiffPixels: 100,
    });
  });

  test('market-data - empty state', async ({ page }) => {
    await page.goto(`${BASE_URL}/market-data?state=empty`);
    await page.waitForLoadState('networkidle');

    await page.waitForSelector('[data-testid="status-bar"], .status-bar, header', { timeout: 5000 }).catch(() => {});

    await expect(page).toHaveScreenshot('market-data-empty.png', {
      fullPage: true,
      maxDiffPixels: 100,
    });
  });
