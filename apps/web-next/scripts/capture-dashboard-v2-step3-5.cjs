...
const OUT_DIR = path.resolve(__dirname, '../../../evidence/assets/dashboard_v2_step3_5');
...
async function assertAIDecisions(page, label) {
  ...
  await page.waitForFunction(() => {
    const items = document.querySelectorAll('[data-testid="ai-decision-item"]').length;
    const empty = document.querySelector('[data-testid="ai-decisions-empty"]');
...
async function main() {
  ...
  await capture({
    filename: '01_fold_1366_right_closed.png',
    rightRailOpen: false,
  });

  await capture({
    filename: '02_fold_1366_right_open.png',
    rightRailOpen: true,
  });
}
