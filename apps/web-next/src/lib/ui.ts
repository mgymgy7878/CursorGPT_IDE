...
/**
 * Z-index sözlüğü - tutarlı katman yönetimi için
 */
export const z = {
  topbar: 40,
  banner: 30,
...
/**
 * DOM ID için güvenli string üretir
 * Geçersiz karakterleri (boşluk, :, /, vb.) tire ile değiştirir
 * @param input - Dönüştürülecek input (string, number, vb.)
 * @returns Geçerli DOM ID string'i
 * @example
 * toDomId("strategy:123/456") // "strategy-123-456"
 * toDomId("test with spaces") // "test-with-spaces"
 * toDomId(123) // "123"
 */
export const toDomId = (input: string | number | undefined | null): string => {
  if (input == null) return '';
  const str = String(input);
  // Geçersiz karakterleri (boşluk, :, /, vb.) tire ile değiştir
  // Sadece alfanumerik, tire ve alt çizgi bırak
