...
export default function Shell({ children }:{ children: React.ReactNode }){
  ...
  const NavItem = ({ href, label }:{ href:string; label:string })=>{
    ...
    return (
      <Link href={href as any} className={`block px-3 py-2 rounded-xl transition ${active? 'bg-neutral-800 font-semibold' : 'hover:bg-neutral-800/50'}`}>{label}</Link>
    );
  };
  return (
    <div className="min-h-screen bg-black text-white">
      <header className="sticky top-0 z-40 border-b border-neutral-800 bg-black/80 backdrop-blur">
        <div className="mx-auto max-w-[1400px] px-4 h-14 flex items-center gap-3">
          <Link href="/" className="font-bold">SPARK</Link>
          <div className="flex-1" />
          <input aria-label="Global Search" placeholder="Ara…" className="hidden md:block px-3 py-1.5 rounded-lg border border-neutral-800 bg-black w-72" />
          <CommandPalette />
          <div aria-label="Profile" className="w-8 h-8 rounded-full bg-neutral-800" />
        </div>
      </header>
