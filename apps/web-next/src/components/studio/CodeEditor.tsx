...
export default function CodeEditor(){
  ...
  function applyParams(p:any){
    ...
    const next = code.replace(/(indicators:\s*\{\s*emaFast:\s*)(\d+)(,\s*emaSlow:\s*)(\d+)/m, `$1${fast}$3${slow}`);
    setCode(next);
  }
  ...
  return (
    <div className="card p-4 border border-neutral-800 rounded-xl bg-black/30">
      ...
      <textarea className="w-full h-64 rounded bg-black/40 border border-neutral-800 font-mono text-xs p-3" value={code} onChange={e=>setCode(e.target.value)} />
      <div className="muted text-xs mt-1">Optimizer’da “Best’i Uygula” ile emaFast/emaSlow değerleri içeri yazılır.</div>
    </div>
  );
}
