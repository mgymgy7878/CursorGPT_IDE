...
export interface PdfExportConfig {
  ...
  title: string;
  user: string;
  maxRecordsPerPage?: number; // Default 50 records per page
}

// Memory-safe PDF writer with streaming output
export async function writePdf(config: PdfExportConfig): Promise<{ success: boolean; bytes: number; filename: string }> {
