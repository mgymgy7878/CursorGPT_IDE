...
export interface OptimizerJob {
  id: string;
  kind: 'live' | 'paper' | 'backtest' | 'optimize';
  priority: 'high' | 'normal' | 'low';
  source: 'api' | 'scheduler' | 'retry';
  payload: any;
  deadline: number;
  createdAt: number;
  startedAt?: number;
  completedAt?: number;
  status: 'enqueued' | 'started' | 'succeeded' | 'failed' | 'canceled';
  retryCount: number;
  maxRetries: number;
}

export class OptimizerQueue {
  private queue: PriorityQueue<OptimizerJob>;
  private agingThreshold: number = 30000; // 30s aging
  private maxQueueDepth: number = 200;

  constructor() {
    this.queue = new PriorityQueue<OptimizerJob>((a, b) => {
      // Priority: high > normal > low
      ...
      if (ageA > this.agingThreshold && ageB <= this.agingThreshold) return -1;
      if (ageB > this.agingThreshold && ageA <= this.agingThreshold) return 1;
      // FIFO for same priority
      return a.createdAt - b.createdAt;
    });
  }

  enqueue(job: OptimizerJob): boolean {
  ...
  getQueueDepthByPriority(): Record<string, number> {
    const depths: Record<string, number> = { high: 0, normal: 0, low: 0 };
