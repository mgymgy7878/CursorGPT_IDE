...
import { installCrashGuard } from "./bootstrap/crash-guard.js";
installCrashGuard();
...
import { registerCompat } from "./utils/register.js";
import metrics from "./plugins/metrics.js";
import guardrailsParams from "./routes/guardrails/params.js";
...
async function start() {
  ...
  // v1.7 Export@Scale Plugin - robust loader
  try {
    // Try multiple import strategies for ESM/CJS compatibility
    let mod;
    try {
      // ESM import with file URL
      const { pathToFileURL } = await import('url');
      const { resolve } = await import('path');
      const pluginPath = resolve(__dirname, '../plugins/export.js');
      mod = await import(pathToFileURL(pluginPath).href);
    } catch (e1) {
      try {
        // Relative import fallback
        mod = await import('../plugins/export.js');
      } catch (e2) {
        // TS import fallback
        mod = await import('../plugins/export');
      }
    }
    
    const plugin = (mod.default || mod.exportPlugin || mod);
